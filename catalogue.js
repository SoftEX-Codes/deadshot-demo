...
if(grid){
 ...
 function render(animate=false){
  ...
  if(animate)window.moxieMotion?.reveal(grid,0,850);
 }
...
if(title){
 ...
 if(!d){const main=document.querySelector('#main-content');main.replaceChildren();const error=el('section','error-state');error.append(el('p','eyebrow','DEVICE NOT FOUND'),el('h1','','Try the collection.'),el('p','','This device is not currently listed.'));const link=el('a','button button-primary','Explore gaming devices');link.href='devices.html';error.append(link);main.append(error);document.title='Device not found — Moxie Gadgets';}
  ...
  document.querySelector('#detail-add-cart').dataset.addCart=d.id;document.querySelector('#device-whatsapp').href='https://wa.me/2349076420186?text='+encodeURIComponent('Hi Moxie Gadgets, I am interested in the '+d.name+' ('+d.memory+'). Please confirm the current price and availability.');
  const button=document.querySelector('#enquiry-button'),panel=document.querySelector('#enquiry-panel');button.addEventListener('click',()=>{panel.hidden=!panel.hidden;button.setAttribute('aria-expanded',String(!panel.hidden));if(!panel.hidden)window.moxieMotion?.reveal(panel,0,900);});
