...
- Catalogue assistant with model/specification/price/budget/comparison answers and WhatsApp referral for unsupported questions. This local guide is deterministic, not an LLM.
- Slow headline phrase changes, restrained reveals, and reduced-motion support.
...
Static checks passed for local page/asset links, unique element IDs, required form controls, JavaScript/TypeScript syntax, and homepage-only 3D loading. Front/back models were rendered through the software rasterizer and inspected. Live browser verification is recorded after deployment. Cloud Auth, RLS, uploads and AI need integration testing after project provisioning; they are not claimed as verified now.
