...
async function start(){
  ...
  function tick(now){frame=0;if(disposed||paused||!visible||document.hidden||drag)return;const elapsed=now-previous;if(elapsed>=(software?1000/24:1000/40)){model.rotation.y+=Math.min(elapsed/1000,.09)*.14;previous=now;draw();}frame=requestAnimationFrame(tick);}
  ...
  try {
    ...
    if(!renderer){const {PhoneSoftwareRenderer}=await import('./vendor/phone-software-renderer.js');renderer=new PhoneSoftwareRenderer();software=true;}
